...
function fncParticularStrPopupSearch() {
	$("#gridStoreSrch2depth").setGridParam({
		datatype : "json",
		postData : {
			srchCompanyCd : $("#companyCd2depth").val(),
			urlDataSp : gvInput.url,
			srchViewStrCd : $("#viewStrCd2depth").val(),
			srchStrNm : $("#strNm2depth").val()
		}
	}).trigger("reloadGrid");	
...
function fncParticularConfirm2depth() {
	fncSetParticularStore(popParam);	
	fncDivPop2Close();
}
...
<div class="r_popup">
	...
	<div class="r_popupCont">
		<div class="r_search multiLine">
    		<input type='hidden' id='companyCd2depth' name='companyCd2depth' value='${companyCd}' />
    		<input type='hidden' id='url2depth' name='url2depth' value='${url}' />  
			<table>
				<tr>
					<th>${smStrCd}</th>
					<td>
			...
			<span class="search">
				<button onclick="javascript:fncParticularStrPopupSearch()">${smRetrieve}</button>
			</span>
		</div>
		<div class='r_grid'>
		...
		<div class="r_pageBtnWrapper">
			...
			<span class="pageNormal">
				<button onclick="javascript:fncParticularConfirm2depth()">${smCnfm}</button>
			</span>
		</div>
	</div>
</div>