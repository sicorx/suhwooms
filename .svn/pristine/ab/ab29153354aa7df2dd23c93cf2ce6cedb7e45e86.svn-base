...
$(document).ready(function(){
	...
	$('input:radio[name="scrAlarmYn"]').eq(0).attr('checked', true);	
	  ...
	  $("#gridStoreMgnt").jqGrid({
	      url:'retrieveTemonStatusList',
	      datatype: "json",
	      mtype: 'POST',
	      colNames:['${smSeq}',  '${smDate}', '${smTime}', '${smContents}', '${smMeasTemp}', '${smUpperLimit}', '${smLowerLimit}', '${smFailInfo}', 
	                'yyyyMmDd', 'hhMin', 'temonId', 'temonType', 'portNo', 'strCd', 'alarm', 'highTemp', 'lowTemp', 'commErr'],
	      colModel:[
	                ...
	                {name:'commErr',index:'commErr', hidden:true}
	           ],
	     jsonReader : {
	          repeatitems:false
	     },
...
function fncChartPopUp(){
	...
	fn_divPop("retrieveTemonTempChart", "팝업", 900, 700, args);
...
function fncSearch() {
	...
	var device = $("#scrPortNo").val();
	
	var arry = device.split(':');
	
	var scrTemonId = arry[0];
	var scrTemonType = arry[1]; 
	var scrPortNo = arry[2];
	
	$("#gridStoreMgnt").setGridParam({
		 postData : {
			 scrStrCd:$("#scrStrCd").val(),
			 scrTemonId:scrTemonId,
			 scrTemonType:scrTemonType,
			 scrPortNo:scrPortNo,
			 scrYyyyMmDd:$("#scrYyyyMmDd").val(),
			 scrAlarmYn:$(':radio[name="scrAlarmYn"]:checked').val()
		 }
...
<form id='popupForm' name='popupForm' method='post'> 
	...
	<input type="hidden" id="popArg3" name="portNo">
