...
function fncRetrieveTemonTemp() {
	...
	$.ajax({
		type : "POST",
		cache : false,
		url : "/retrieveTemonTempChartData",
		data : {
			strCd : strCd,
			yyyyMmDd  : yyyyMmDd,
			temonId  : scrTemonId,
			temonType  : scrtemonType,
			portNo : scrPortNo
		},
		dataType : "json",
...
<div class="r_popup">
	...
	<div class="r_popupCont">	
		<div class="r_search multiLine">  
			...
			<span class="search">
				<button onclick="javascript:fncRetrieveTemonTemp()">${smRetrieve}</button>
			</span>
		</div>
