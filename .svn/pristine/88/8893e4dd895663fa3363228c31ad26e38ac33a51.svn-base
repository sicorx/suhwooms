...
$(document).ready(function(){
	...
	$("#gridList").jqGrid({
	      ...
	      colModel:[
			    	  ...
			    	  {name:'strNm',                index:'strNm',                  width : "100%", align:'center', sortable:true,  editable:false, formatter: strFormatter},
            	 ...
            	 if(status == "modifyNumber"){
                     ...
                     $("#gridList").jqGrid('setColProp', 'cntCt', {editable:true});
	           	    ...
	           	    for(var i = 0; i < ids.length; i++) {
	           	        $("#gridList").jqGrid("editRow", ids[i+1]);
	           	    }
            	 }
             }
         },
...
function fncUpdateModifyClosingMon() {
	...
	for(var i = 1; i <= ids.length; i++){
        ...
        var strCd = $("#gridList").getRowData(i).strCd;
        param.strCd = strCd.replace("-", "");
        param.closingYm_new = sel[i-1].value;
        ...
        <form id="saveForm" name="saveForm" method="post">
			<table>
				<tr>
				    <th>매장명/코드</th>
	                <td>
	                    <input type="search" id="searchVal" name="searchVal" value="" style="width:118px;" />
	                </td>
					<th>설치일자</th>
					<td colspan="7">
