...
public class UserMgntCtrl {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(UserMgntCtrl.class);

	@Autowired
	private UserMgntSvc userMgntSvc;
	
	@Autowired
	private AuthMgntSvc authSvc;
	...
	public String showUserMgnt(
		...
		authParams.put("companyCd", sessionUserVo.getSessionCompanyCd());		
		List<AuthVo> authList = authSvc.retrieveAuthListDtl(authParams);		
				
				
		// commCdList 가지고 오기
		ComCodeVo commcdParams = new ComCodeVo();
		commcdParams.setCommGrpCd("MAST04");
		List<ComCodeVo> commCdList = commonSvc.retrieveComCodeList(commcdParams);
		
		// vendorList 가지고 오기		
		List<SelectVo> vendorList = vsmSvc.retrieveVendorList(params);
		
		// electricList 가지고 오기		
		List<SelectVo> electricEntList = userMgntSvc.retrieveElectricEntList(params);		
			
		JSONArray jsonArray = new JSONArray();
		//model.addAttribute("authList", JSONArray.fromObject(authList) );
		model.addAttribute("authList", JSONArray.fromObject(authList) );
		model.addAttribute("commCdList", JSONArray.fromObject(commCdList));
		model.addAttribute("vendorCdList", JSONArray.fromObject(vendorList) );
		model.addAttribute("electricEntList", JSONArray.fromObject(electricEntList) );		
	...
	public @ResponseBody GridVo<GridUserMgntVo> retrieveUserMgnt(
		...
		params.put( "commcdParams", "MAST04" );		
		
		// commCdList 가지고 오기
/*		ComCodeVo commcdParams = new ComCodeVo();
		commcdParams.setCommGrpCd("MAST04");
		List<ComCodeVo> commCdList = commonSvc.retrieveComCodeList(commcdParams);
		int commCdCnt = commCdList.size();		
		
		// vendorList 가지고 오기		
		List<SelectVo> vendorList = vsmSvc.retrieveVendorList(params);
		int vendorCnt = vendorList.size();
		
		// electricList 가지고 오기		
		List<SelectVo> electricEntList = userMgntSvc.retrieveElectricEntList(params);
		int electricEntCnt = electricEntList.size();*/
	...
	public String retrieveUserMgntDtl(
		...
		List<SelectVo> rslt6 = userMgntSvc.retrieveElectricEntList( params );
		...
		model.addAttribute("selectVoList5", rslt6);
	...
	public String showUserMgntNew(
		...
		List<SelectVo> rslt5 = userMgntSvc.retrieveAuthTList( companyCd );
		...
		model.addAttribute("selectVoList5", rslt5);
		model.addAttribute("selectVoList6", rslt6);
	...
	public @ResponseBody boolean saveUserMgnt(
			@ModelAttribute("sessionUserVo") SessionUserVo sessionUserVo,
		...
		LOGGER.info( gumVo.getStrCd() );
		LOGGER.info( gumVo.getStrNm() );
	...
	public @ResponseBody boolean deleteUserMgnt(
			@ModelAttribute("sessionUserVo") SessionUserVo sessionUserVo,
			@RequestParam(value = "userId", required = false) String userId,
			@RequestParam(value = "companyCd", required = false) String companyCd,			
			BindingResult bindingResult,
		...
		boolean rslt1 = userMgntSvc.deleteRemsUserMgnt( params );					// th_user 에서 제거
		boolean rslt2 = userMgntSvc.deleteRemsUserAuthMgnt( params );				// th_user_auth 에서 제거
