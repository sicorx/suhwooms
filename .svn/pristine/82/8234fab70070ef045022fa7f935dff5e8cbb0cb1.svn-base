...
$(document).ready(function() {
	...
	$('#stopWeek0').on("click",function(){
		
		var sta = $('#stopWeek0').is(":checked");
		
		if( sta == true )
		{
			$("input[name='stopWeek[]']:checkbox").each(function() {
...
function fncStoreConfigSetting(){
	...
	$.ajax({
		...
		success:function(data){
			...
			var arryOld = data.strConfValOld.split(',');
			var arryOld2 = arry[0].split(':');
			var arryOld3 = arry[1].split(':');
	...
	$.ajax({
		...
		success:function(data){
			...
			$("#so1").val(arry[0]);
			$("#so2").val(arry[1]);
			$("#signOperOld").val(data.strConfVal);
		},
		error:function(result){
			$("#so1").val("08:00");			// 디폴트 값 셋팅
			$("#so2").val("08:00");			// 디폴트 값 셋팅
			$("#signOperOld").val("08:00,08:00");			// 디폴트 값 셋팅
		}
	});
	...
	$.ajax({
		...
		dataType:'json',
		success:function(data){
			$("#signStowOld").val(data.strConfVal);
			
			var arry = data.strConfVal.split(',');
						
			var obj = document.getElementsByName("sw[]");
			...
			{
				obj[ arry[0] ].checked = true;
				obj[ arry[1] ].checked = true;
			}
		},
		error:function(result){
			obj[ 0 ].check = true;			
	...
	$.ajax({
		...
		success:function(data){
			...
			var obj = document.getElementsByName("st[]");
...
function fncUpt() {
	...
	var so1 = $("#so1").val();								// 영업 시작 / 종료 일시
	var so2 = $("#so2").val();
	param.signOper = so1 + "," + so2;
	param.signOperOld = $("#signOperOld").val();
	
	
	var objSw = document.getElementsByName("sw[]");			// 영업 휴무 주
	...
	param.signStow =  swsw;						
	param.signStowOld =  $("#signStowOld").val();
	
	
	var std1 = $("#std1").val();							// 영업 휴무 일							
	var std2 = $("#std2").val();							
	param.signStod =  std1 +","+ std2;						
	param.signStodOld =  $("#stdOld").val();
	
	//alert( "std : " + param.signStod );
	
	
	var objSt = document.getElementsByName("st[]");			// 영업 휴무 요일
	...
	param.signStot =  stst;
	param.signStotOld =  $("#stOld").val();
	
	//alert( "st : " + param.signStot );
				        ...
				        <tbody>
							...
							<tr>
								<th colspan="3" style="width:30%">
									영업 시작/종료									
								<span class="r_required">*</span></th>								
								<td colspan="2"  class="tleft">
									<span style="padding:10px">시작 시간 : </span>
									<input type='text' id='so1' name='so1' style="width:50px;" value="08:00,08:00" required/>
									<span style="padding:10px">종료 시간 : </span>
									<input type='text' id='so2' name='so2' style="width:50px;" value="08:00,08:00" required/>
									<input type='hidden' id='signOperOld' name='signOperOld' />
									
									<br>
									＊ 시작시간과 종료시간이 같을경우 24시간 영업
								</td>
							</tr>
							
							
							<tr>
								<th colspan="3" style="width:30%">휴무 주<span class="r_required">*</span></th>
								<td colspan="2" class="tleft">
									<input type="checkbox" id="sw0" name="sw[]" value="0" /><label for="sw0">없음</label>
									<input type="checkbox" id="sw1" name="sw[]" value="1" /><label for="sw1">1째주</label>								
									<input type="checkbox" id="sw2" name="sw[]" value="2" /><label for="sw2">2째주</label>
									<input type="checkbox" id="sw3" name="sw[]" value="3" /><label for="sw3">3째주</label>
									<input type="checkbox" id="sw4" name="sw[]" value="4" /><label for="sw4">4째주</label>
									<input type="checkbox" id="sw5" name="sw[]" value="5" /><label for="sw5">5째주</label>
									<input type='hidden' id='signStowOld' name='signStowOld' />
								</td>
							</tr>
							<tr>
								<th colspan="3" style="width:30%">휴무 일<span class="r_required">*</span></th>
								<td colspan="2" class="tleft">
									<span style="padding:10px">휴무 일1 </span>
									<input type='text' id='std1' name='std1' style="width:50px;" value="0" required/>
									<span style="padding:10px">휴무 일2 </span>
									<input type='text' id='std2' name='std2' style="width:50px;" value="0" required/>
									<input type='hidden' id='stdOld' name='stdOld' />
									<br>
									＊ 휴무 일1, 휴무 일2 가 둘다 0일 일경우 휴무없음
								</td>
							</tr>
							<tr>
								<th colspan="3" style="width:30%">휴무 요일<span class="r_required">*</span></th>
								<td colspan="2" class="tleft">
									<input type="checkbox" id="st0" name="st[]" value="0" /><label for="st0">없음</label>
									<input type="checkbox" id="st1" name="st[]" value="1" /><label for="st1">월요일</label>								
									<input type="checkbox" id="st2" name="st[]" value="2" /><label for="st2">화요일</label>
									<input type="checkbox" id="st3" name="st[]" value="3" /><label for="st3">수요일</label>
									<input type="checkbox" id="st4" name="st[]" value="4" /><label for="st4">목요일</label>
									<input type="checkbox" id="st5" name="st[]" value="5" /><label for="st5">금요일</label>
									<input type="checkbox" id="st6" name="st[]" value="6" /><label for="st6">토요일</label>
									<input type="checkbox" id="st7" name="st[]" value="7" /><label for="st7">일요일</label>
									<input type='hidden' id='stOld' name='stOld' />
								</td>
							</tr>
