...
$(document).ready(function() {
    ...
    if('${newYn}' == "N" ){
    	...
    	$('input[type=radio][name=authSp]').on('change', function(){
    		...
    		}else if( '${userMgntDtl.authSp}'  == 'T' ){
    			$('#authSp_T').prop('checked', true);
    		}
    	});
    	
    	
    	
    	if( '${userMgntDtl.authSp}'  == 'H' ){				// 본사직원
    		$('#authSp_H').prop('checked', true);
            $('#orgPopup').show();
            $('#strPopup').hide();
            $('#hAuth').show();
            ...
            $('#tAuth').hide();            
            $('#vendorList').hide();
            $('#electricEntList').hide();
    	}else if( '${userMgntDtl.authSp}'  == 'M' ){		// 유지보수 업체
    		$('#authSp_M').prop('checked', true);
        	$('#orgPopup').hide();
        	$('#strPopup').hide();
        	$('#hAuth').hide();
        	...
        	$('#vendorList').show();
        	$('#electricEntList').hide();
    	}else if( '${userMgntDtl.authSp}'  == 'S' ){		// 매장직원
    		$('#authSp_S').prop('checked', true);
        	$('#orgPopup').hide();
        	$('#strPopup').show();
        	$('#hAuth').hide();
        	...
        	$('#vendorList').hide();
        	$('#electricEntList').hide();
    	}else if( '${userMgntDtl.authSp}'  == 'T' ){		// 전기업체
    		$('#authSp_T').prop('checked', true);
    		$('#orgPopup').hide();
        	$('#strPopup').hide();
        	$('#hAuth').hide();
			...
			<form id="saveForm" name="saveForm" method="post">
				<table>
					...
					<tr>
						...
						<td>
							<input type='password' id='userPw' name='userPw' maxlength="8"  value="${userMgntDtl.userPw}" style="width:145px;" required/>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<span class="r_radio">
						    	...
						    	&nbsp;
						    	<input type='radio' name='authSp' id='authSp_T' value='T' /><label for="authSp_T">전기 업체</label>
					...
					<tr id="tAuth" style="display:none">
						<th>사용자 권한<span class="r_required">*</span></th>
						<td>
							<select id="authCd4" name="authCd4" style="width:130px;" class="tblSelect">
							<option value=''>--선택--</option>								
								<c:forEach var="vo" items="${selectVoList5}" varStatus="status" >
									<option value='${vo.name}' <c:if test="${userMgntDtl.authCd == vo.name}">selected</c:if> >${vo.value}</option>
								</c:forEach>								
							</select>
						</td>
					...
					<tr id="electricEntList" style="display:none">
						<th>업체명<span class="r_required">*</span></th>
						<td>
							<select id="electricEntCd" name="electricEntCd" style="width:130px;" class="tblSelect">
							<option value=''>--선택--</option>
								<c:forEach var="vo" items="${selectVoList6}" varStatus="status" >
									<option value='${vo.value}' <c:if test="${userMgntDtl.vendorCd == vo.value}">selected</c:if> >${vo.name}</option>
								</c:forEach>
							</select>
						</td>
